...
const CONDITION_CONFIG: Record<string, { bg: string; text: string; border: string; label: string; description: string }> = {
    UNUSED: { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', label: 'Unused', description: 'Material was not used and is in original condition' },
    PARTIALLY_USED: { bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-200', label: 'Partially Used', description: 'Material was partially consumed in production' },
    CONTAMINATED: { bg: 'bg-red-50', text: 'text-red-700', border: 'border-red-200', label: 'Contaminated', description: 'Material is contaminated and may not be reusable' },
};

const ROUTED_TO_CONFIG: Record<string, { bg: string; text: string; border: string; label: string }> = {
    AVAILABLE: { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', label: 'Available Stock' },
    QC_HOLD: { bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-200', label: 'QC Hold' },
    REJECT: { bg: 'bg-red-50', text: 'text-red-700', border: 'border-red-200', label: 'Rejected' },
};
...
export function MaterialReturnScreen() {
    ...
    const { data, isLoading } = useMaterialReturns(params);
    ...
    return (
        <div className="flex-1 p-6 max-w-7xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Material Return from Production</h1>
                    <p className="text-sm text-neutral-500 dark:text-neutral-400">Return unused or surplus materials from production floor</p>
                </div>
                {canCreate && (
            ...
            <div className="flex items-center gap-3 flex-wrap">
                <div className="relative flex-1 min-w-[200px] max-w-sm">
                    ...
                    <input
                        ...
                        placeholder="Search return or WO number..."
                        value={search}
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                                    ...
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Condition</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Routed To</th>
                                ...
                                {items.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="text-center py-12">
                                            ...
                                            <p className="text-sm text-neutral-400">No material returns found</p>
                                        </td>
                                    </tr>
                                )}
                                {items.map((item: any) => {
                                    ...
                                    return (
                                        <tr key={item.id} className="border-b border-neutral-50 dark:border-neutral-800/50 hover:bg-neutral-50 dark:hover:bg-neutral-800/30">
                                            ...
                                            <td className="px-4 py-3">
                                                ...
                                                <p className="text-[10px] text-neutral-500 truncate max-w-[140px]">{item.partName || item.part?.name || ''}</p>
                                            </td>
                                            <td className="px-4 py-3">
                                                <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full border', condConfig.bg, condConfig.text, condConfig.border)}>
                                                    {condConfig.label}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3">
                                                <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full border', routeConfig.bg, routeConfig.text, routeConfig.border)}>
                                                    {routeConfig.label}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3 text-right font-medium text-neutral-900 dark:text-white">{Number(item.returnQty ?? item.quantity ?? 0).toLocaleString()}</td>
            ...
            {showModal && <CreateMaterialReturnModal onClose={() => setShowModal(false)} />}
        </div>
    );
}
...
function CreateMaterialReturnModal({ onClose }: { onClose: () => void }) {
    ...
    const routeConfig = ROUTED_TO_CONFIG[routedTo] || ROUTED_TO_CONFIG.AVAILABLE;
    const canSubmit = form.workOrderId && form.partId && form.returnQty > 0;
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-xl w-full max-w-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold text-neutral-900 dark:text-white">New Material Return</h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input className={inputClass} value={form.partId} onChange={(e) => setForm({ ...form, partId: e.target.value })} placeholder="Part issued to this WO" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-neutral-400 mb-1">Return Qty *</label>
                        <input type="number" min={1} className={inputClass} value={form.returnQty} onChange={(e) => setForm({ ...form, returnQty: Number(e.target.value) })} />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-neutral-400 mb-1">Lot Number</label>
                        <input className={inputClass} value={form.lotNumber} onChange={(e) => setForm({ ...form, lotNumber: e.target.value })} placeholder="From original issue" />
                    </div>
                </div>

                {/* Return Condition */}
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-neutral-400 mb-2">Return Condition *</label>
                    <div className="grid grid-cols-3 gap-3">
                        {Object.entries(CONDITION_CONFIG).map(([key, config]) => (
                            <button
                                key={key}
                                type="button"
                                onClick={() => setForm({ ...form, returnCondition: key })}
                                className={cn(
                                    'rounded-xl border-2 p-3 text-left transition-all',
                                    form.returnCondition === key
                                        ? `${config.bg} ${config.border} ring-2 ring-offset-1 ring-${config.text.split('-')[1]}-300`
                                        : 'border-neutral-200 dark:border-neutral-700 hover:border-neutral-300',
                                )}
                            >
                                <p className={cn('text-xs font-bold', form.returnCondition === key ? config.text : 'text-neutral-700 dark:text-neutral-300')}>
                                    {config.label}
                                </p>
                                <p className="text-[10px] text-neutral-500 mt-0.5">{config.description}</p>
                            </button>
                        ))}
                    </div>
                </div>
                ...
                <div className={cn('px-4 py-3 rounded-xl border text-xs font-medium', routeConfig.bg, routeConfig.border, routeConfig.text)}>
                    Stock will be routed to: <span className="font-bold">{routeConfig.label}</span>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    <button onClick={handleSubmit} disabled={!canSubmit || createMutation.isPending} className="px-4 py-2 rounded-lg bg-primary-600 text-white text-sm font-semibold hover:bg-primary-700 disabled:opacity-50 transition-colors">
                        {createMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Submit Return'}
                    </button>
                </div>
            </div>
        </div>
