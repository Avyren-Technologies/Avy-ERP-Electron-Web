...
import { Loader2, Plus, ArrowRightLeft, Search, X, Trash2, CheckCircle2 } from 'lucide-react';
import { useCanPerform } from '@/hooks/useCanPerform';
import { useCompanyFormatter } from '@/hooks/useCompanyFormatter';
import { useMoveStock, useWarehouses } from '@/features/inventory/api/use-inventory-queries';
...
export function MoveStockScreen() {
    ...
    const confirmReceipt = useConfirmMoveReceipt();
    ...
    const { data, isLoading } = useMoveStock(params);
    ...
    return (
        <div className="flex-1 p-6 max-w-7xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Stock Transfer</h1>
                    <p className="text-sm text-neutral-500 dark:text-neutral-400">Move stock between warehouses and locations</p>
                </div>
                {canCreate && (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                                    ...
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">From Warehouse</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">To Warehouse</th>
                                ...
                                {items.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="text-center py-12">
                                            <ArrowRightLeft className="w-8 h-8 text-neutral-300 mx-auto mb-2" />
                                            <p className="text-sm text-neutral-400">No stock transfers found</p>
                                        </td>
                                    </tr>
                                )}
                                {items.map((item: any) => (
                                    <tr key={item.id} className="border-b border-neutral-50 dark:border-neutral-800/50 hover:bg-neutral-50 dark:hover:bg-neutral-800/30">
                                        ...
                                        <td className="px-4 py-3 text-neutral-700 dark:text-neutral-300 text-xs">{item.fromWarehouse?.code || item.fromWarehouseCode || '--'}</td>
                                        <td className="px-4 py-3 text-neutral-700 dark:text-neutral-300 text-xs">{item.toWarehouse?.code || item.toWarehouseCode || '--'}</td>
                                        <td className="px-4 py-3 text-right font-medium text-neutral-900 dark:text-white">{item.lineItems?.length || item.itemCount || 0}</td>
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            {item.status === 'IN_TRANSIT' && (
                                                <button
                                                    onClick={() => confirmReceipt.mutate(item.id)}
            ...
            {showModal && <CreateMoveModal onClose={() => setShowModal(false)} />}
        </div>
    );
}

function CreateMoveModal({ onClose }: { onClose: () => void }) {
    const createMutation = useCreateMoveStock();
    const [srcWarehouseId, setSrcWarehouseId] = useState('');
    const [srcZoneId, setSrcZoneId] = useState('');
    const [srcBinId, setSrcBinId] = useState('');
    const [dstWarehouseId, setDstWarehouseId] = useState('');
    const [dstZoneId, setDstZoneId] = useState('');
    const [dstBinId, setDstBinId] = useState('');
    ...
    const canSubmit = srcWarehouseId && dstWarehouseId && lineItems.every(li => li.partId && li.qty > 0);

    const handleSubmit = () => {
        createMutation.mutate({
            fromWarehouseId: srcWarehouseId,
            fromZoneId: srcZoneId || undefined,
            fromBinId: srcBinId || undefined,
            toWarehouseId: dstWarehouseId,
            toZoneId: dstZoneId || undefined,
            toBinId: dstBinId || undefined,
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-xl w-full max-w-4xl p-6 space-y-5 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold text-neutral-900 dark:text-white">New Stock Transfer</h3>
                ...
                <div className="space-y-4">
                    <div>
                        <p className="text-sm font-medium text-gray-700 dark:text-neutral-400 mb-2">Source Location</p>
                        <WarehouseLocationPicker
                            warehouseId={srcWarehouseId} zoneId={srcZoneId} binId={srcBinId}
                            onWarehouseChange={setSrcWarehouseId} onZoneChange={setSrcZoneId} onBinChange={setSrcBinId}
                            required
                        />
                    </div>
                    <div className="flex justify-center"><ArrowRightLeft className="w-5 h-5 text-neutral-400 rotate-90" /></div>
                    <div>
                        <p className="text-sm font-medium text-gray-700 dark:text-neutral-400 mb-2">Destination Location</p>
                        <WarehouseLocationPicker
                            warehouseId={dstWarehouseId} zoneId={dstZoneId} binId={dstBinId}
                            onWarehouseChange={setDstWarehouseId} onZoneChange={setDstZoneId} onBinChange={setDstBinId}
                            required
                        />
                    </div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    <button onClick={handleSubmit} disabled={!canSubmit || createMutation.isPending} className="px-4 py-2 rounded-lg bg-primary-600 text-white text-sm font-semibold hover:bg-primary-700 disabled:opacity-50 transition-colors">
                        {createMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Create Transfer'}
                    </button>
                </div>
            </div>
        </div>
