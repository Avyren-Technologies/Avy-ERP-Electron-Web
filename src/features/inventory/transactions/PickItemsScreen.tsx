...
import { Loader2, Plus, PackageSearch, Search, X, Trash2, CheckCircle2 } from 'lucide-react';
import { useCanPerform } from '@/hooks/useCanPerform';
import { useCompanyFormatter } from '@/hooks/useCompanyFormatter';
import { usePickItems, useWarehouses } from '@/features/inventory/api/use-inventory-queries';
...
export function PickItemsScreen() {
    ...
    const { data, isLoading } = usePickItems(params);
    ...
    return (
        <div className="flex-1 p-6 max-w-7xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Pick & Issue</h1>
                    <p className="text-sm text-neutral-500 dark:text-neutral-400">Manage pick lists for sales orders, work orders, and projects</p>
                </div>
                {canCreate && (
            ...
            <div className="flex items-center gap-3 flex-wrap">
                <div className="relative flex-1 min-w-[200px] max-w-sm">
                    ...
                    <input
                        ...
                        placeholder="Search pick list number..."
                        value={search}
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                                    ...
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Parts</th>
                                ...
                                {items.length === 0 && (
                                    <tr>
                                        <td colSpan={7} className="text-center py-12">
                                            ...
                                            <p className="text-sm text-neutral-400">No pick lists found</p>
                                        </td>
                                    </tr>
                                )}
                                {items.map((item: any) => (
                                    <tr key={item.id} className="border-b border-neutral-50 dark:border-neutral-800/50 hover:bg-neutral-50 dark:hover:bg-neutral-800/30">
                                        ...
                                        <td className="px-4 py-3 text-neutral-700 dark:text-neutral-300 text-xs">
                                            {item.lineItems?.map((li: any) => li.partNumber || li.part?.partNumber).filter(Boolean).join(', ') || `${item.itemCount || 0} items`}
                                        </td>
                                        <td className="px-4 py-3 text-right font-medium text-neutral-900 dark:text-white">
                                            {item.lineItems?.reduce((s: number, li: any) => s + Number(li.quantity ?? li.qty ?? 0), 0)?.toLocaleString() || '--'}
                                        </td>
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            {(item.status === 'SUBMITTED' || item.status === 'APPROVED') && (
                                                <button
                                                    onClick={() => confirmPick.mutate({ id: item.id, data: {} })}
                                                ...
                                                >
                                                    <CheckCircle2 className="w-3.5 h-3.5" /> Confirm Pick
                                                </button>
                                            )}
                                        </td>
                                    </tr>
            ...
            {showModal && <CreatePickModal onClose={() => setShowModal(false)} />}
        </div>
    );
}
...
function CreatePickModal({ onClose }: { onClose: () => void }) {
    ...
    const canSubmit = warehouseId && lineItems.every(li => li.partId && li.qty > 0);

    const handleSubmit = () => {
        createMutation.mutate({
            warehouseId,
            demandReference: demandReference || undefined,
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-xl w-full max-w-2xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold text-neutral-900 dark:text-white">New Pick List</h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-neutral-400 mb-1">Demand Reference</label>
                        <input className={inputClass} value={demandReference} onChange={(e) => setDemandReference(e.target.value)} placeholder="SO/WO/Project" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p className="text-[10px] text-neutral-400 mt-1">FEFO/FIFO allocation is applied automatically for lot-tracked items</p>
                </div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    <button onClick={handleSubmit} disabled={!canSubmit || createMutation.isPending} className="px-4 py-2 rounded-lg bg-primary-600 text-white text-sm font-semibold hover:bg-primary-700 disabled:opacity-50 transition-colors">
                        {createMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Create Pick List'}
                    </button>
                </div>
            </div>
        </div>
