...
import { Loader2, Plus, Truck, Search, X, Trash2 } from 'lucide-react';
    ...
    return (
        <div className="flex-1 p-6 max-w-7xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Pack & Dispatch</h1>
                    <p className="text-sm text-neutral-500 dark:text-neutral-400">Manage outgoing shipments and deliveries</p>
                </div>
                {canCreate && (
            ...
            <div className="flex items-center gap-3 flex-wrap">
                <div className="relative flex-1 min-w-[200px] max-w-sm">
                    ...
                    <input
                        ...
                        placeholder="Search dispatch number or SO..."
                        value={search}
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Dispatch #</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">SO Reference</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Customer</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Vehicle #</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Status</th>
                                    <th className="text-left px-4 py-3 text-xs font-semibold text-neutral-500 uppercase tracking-wider">Dispatched At</th>
                                </tr>
                            </thead>
                            <tbody>
                                {items.length === 0 && (
                                    <tr>
                                        <td colSpan={6} className="text-center py-12">
                                            ...
                                            <p className="text-sm text-neutral-400">No dispatches found</p>
                                        </td>
                                    </tr>
                                )}
                                {items.map((item: any) => (
                                    <tr key={item.id} className="border-b border-neutral-50 dark:border-neutral-800/50 hover:bg-neutral-50 dark:hover:bg-neutral-800/30">
                                        <td className="px-4 py-3 font-mono text-xs font-bold text-primary-600 dark:text-primary-400">{item.dispatchNumber || item.transactionNumber || '--'}</td>
                                        <td className="px-4 py-3 text-neutral-700 dark:text-neutral-300 text-xs">{item.soReference || item.salesOrderNumber || '--'}</td>
                                        <td className="px-4 py-3 text-neutral-700 dark:text-neutral-300 text-xs">{item.customerName || item.customer?.name || '--'}</td>
                                        <td className="px-4 py-3 text-neutral-700 dark:text-neutral-300 text-xs">{item.vehicleNumber || '--'}</td>
                                        <td className="px-4 py-3"><TransactionStatusBadge status={item.status || 'DRAFT'} /></td>
                                        <td className="px-4 py-3 text-neutral-700 dark:text-neutral-300 text-xs">{item.dispatchedAt ? fmt.dateTime(item.dispatchedAt) : item.createdAt ? fmt.dateTime(item.createdAt) : '--'}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
            ...
            {showModal && <CreateDispatchModal onClose={() => setShowModal(false)} />}
        </div>
    );
}
...
function CreateDispatchModal({ onClose }: { onClose: () => void }) {
    ...
    const canSubmit = lineItems.every(li => li.partId && li.qty > 0);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-xl w-full max-w-3xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between">
                    <h3 className="text-lg font-bold text-neutral-900 dark:text-white">New Dispatch</h3>
                ...
                <div className="grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <input className={inputClass} value={form.soReference} onChange={(e) => setForm({ ...form, soReference: e.target.value })} placeholder="SO-00001" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-neutral-400 mb-1">Customer</label>
                        <input className={inputClass} value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })} placeholder="Customer name" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-neutral-400 mb-1">Vehicle Number</label>
                        <input className={inputClass} value={form.vehicleNumber} onChange={(e) => setForm({ ...form, vehicleNumber: e.target.value })} placeholder="KA-01-XX-1234" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-neutral-400 mb-1">Driver Name</label>
                        <input className={inputClass} value={form.driverName} onChange={(e) => setForm({ ...form, driverName: e.target.value })} placeholder="Driver" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-neutral-400 mb-1">E-Way Bill</label>
                        <input className={inputClass} value={form.ewayBillNumber} onChange={(e) => setForm({ ...form, ewayBillNumber: e.target.value })} placeholder="EWB number" />
                    </div>
                    <div>
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    <button onClick={handleSubmit} disabled={!canSubmit || createMutation.isPending} className="px-4 py-2 rounded-lg bg-primary-600 text-white text-sm font-semibold hover:bg-primary-700 disabled:opacity-50 transition-colors">
                        {createMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Create Dispatch'}
                    </button>
                </div>
            </div>
        </div>
