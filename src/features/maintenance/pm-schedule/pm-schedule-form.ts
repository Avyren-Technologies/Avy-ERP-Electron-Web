...
/** PM reschedule history row from API (`oldDueDate` / `newDueDate` / `rescheduledAt`). */
...
/** Map API PM schedule → form state for edit screens. */
export function pmScheduleToFormState(pm: Record<string, unknown>): PMScheduleFormState {
    ...
    return {
        ...
        customIntervalDays:
            pm.customIntervalDays != null ? String(pm.customIntervalDays) : "30",
...
/** PATCH payload (asset cannot be changed on update). */
