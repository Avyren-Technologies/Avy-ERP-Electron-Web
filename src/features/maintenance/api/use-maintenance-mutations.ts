...
import { maintenanceKeys } from './use-maintenance-queries';
...
export function useApproveWorkRequest() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id }: { id: string }) => maintenanceApi.approveWorkRequest(id),
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.workRequest(variables.id) });
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.workRequests() });
        },
...
export function useConvertWorkRequest() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id }: { id: string }) => maintenanceApi.convertWorkRequest(id),
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.workRequest(variables.id) });
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.workRequests() });
        },
...
export function useCancelWorkRequest() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id }: { id: string }) => maintenanceApi.cancelWorkRequest(id),
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.workRequest(variables.id) });
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.workRequests() });
        },
...
function useWOAction(
    actionFn: (args: { id: string; data?: any }) => Promise<any>,
) {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: actionFn,
...
export function useAcknowledgeWO() {
    return useWOAction(({ id }) => maintenanceApi.acknowledgeWorkOrder(id));
}

export function useDeclineWO() {
    return useWOAction(({ id, data }) => maintenanceApi.declineWorkOrder(id, data));
}

export function useStartWO() {
    return useWOAction(({ id }) => maintenanceApi.startWorkOrder(id));
}

export function useHoldWO() {
    return useWOAction(({ id, data }) => maintenanceApi.holdWorkOrder(id, data));
}

export function useResumeWO() {
    return useWOAction(({ id }) => maintenanceApi.resumeWorkOrder(id));
}

export function useCompleteWO() {
    return useWOAction(({ id, data }) => maintenanceApi.completeWorkOrder(id, data));
}

export function useQAReleaseWO() {
    return useWOAction(({ id }) => maintenanceApi.qaReleaseWorkOrder(id));
}

export function useCloseWO() {
    return useWOAction(({ id, data }) => maintenanceApi.closeWorkOrder(id, data));
}

export function useRejectWO() {
    return useWOAction(({ id, data }) => maintenanceApi.rejectWorkOrder(id, data));
}

export function useCancelWO() {
    return useWOAction(({ id }) => maintenanceApi.cancelWorkOrder(id));
}

export function useReopenWO() {
    return useWOAction(({ id, data }) => maintenanceApi.reopenWorkOrder(id, data));
}
...
export function useReschedulePM() {
    ...
    return useMutation({
        ...
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.pmSchedule(variables.id) });
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.pmSchedules() });
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.overduePMs() });
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.dueTodayPMs() });
        },
    });
}
...
export function useGenerateWOFromPM() {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id }: { id: string }) => maintenanceApi.generateWOFromPM(id),
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.pmSchedule(variables.id) });
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.pmSchedules() });
            queryClient.invalidateQueries({ queryKey: maintenanceKeys.workOrders() });
