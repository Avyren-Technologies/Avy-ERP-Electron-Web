// Step 13 — No. Series (Document Numbering)
...
function parseIntOrDefault(v: unknown, fallback: number) {
    const n = typeof v === 'number' ? v : parseInt(String(v ?? ''), 10);
    return Number.isNaN(n) ? fallback : n;
}

function NoSeriesItemCard({
    idx,
    control,
    remove,
}: {
    ...
    const code = useWatch({ control, name: `noSeries.${idx}.code` }) as string | undefined;
    const linkedScreen = useWatch({ control, name: `noSeries.${idx}.linkedScreen` }) as string | undefined;
    const prefix = useWatch({ control, name: `noSeries.${idx}.prefix` }) as string | undefined;
    const suffix = useWatch({ control, name: `noSeries.${idx}.suffix` }) as string | undefined;
    const numberCount = useWatch({ control, name: `noSeries.${idx}.numberCount` }) as string | undefined;
    const startNumber = useWatch({ control, name: `noSeries.${idx}.startNumber` }) as string | undefined;

    const { errors } = useFormState({ control });
    const itemErrors = (errors as any)?.noSeries?.[idx] as any | undefined;

    const count = parseIntOrDefault(numberCount, 4);
    const start = parseIntOrDefault(startNumber, 1);
    const variableNumber = String(start).padStart(count, '0');

    // Preview format: Prefix + Suffix + Variable Number
    const hasPrefixOrSuffix = (prefix ?? '').trim().length > 0 || (suffix ?? '').trim().length > 0;
    const preview = `${prefix ?? ''}${suffix ?? ''}${variableNumber}`;

    return (
        <ItemCard
            title={code || `Series ${idx + 1}`}
            subtitle={linkedScreen || 'No document type selected'}
            badge={`${idx + 1}`}
            defaultOpen={idx === 0}
        >
            <div className="space-y-4">
                <TwoCol>
                    <Controller name={`noSeries.${idx}.code`} control={control} render={({ field: subField }) => (
                        <FormInput
                            ...
                            onChange={(e) => subField.onChange(e.toUpperCase())}
                            required
                            monospace
                            hint="Unique identifier for this series (used in API + reports)"
                            error={itemErrors?.code?.message}
                        />
                    )} />
                </TwoCol>
                ...
                <Controller name={`noSeries.${idx}.linkedScreen`} control={control} render={({ field: subField }) => (
                    <FormSelect
                        ...
                        placeholder="Select linked screen"
                        searchable
                        required
                        error={itemErrors?.linkedScreen?.message}
                    />
                )} />
                ...
                <ThreeCol>
                    <Controller name={`noSeries.${idx}.prefix`} control={control} render={({ field: subField }) => (
                        <FormInput
                            ...
                            value={subField.value || ''}
                            monospace
                            hint="Text before the variable number"
                            error={itemErrors?.prefix?.message}
                        />
                    )} />
                    <Controller name={`noSeries.${idx}.suffix`} control={control} render={({ field: subField }) => (
                        <FormInput
                            ...
                            value={subField.value || ''}
                            monospace
                            hint="Fixed segment placed before the variable number"
                            error={itemErrors?.suffix?.message}
                        />
                    )} />
                    <Controller name={`noSeries.${idx}.numberCount`} control={control} render={({ field: subField }) => (
                        <FormInput
                            ...
                            error={itemErrors?.numberCount?.message}
                        />
                    )} />
                </ThreeCol>
                ...
                <TwoCol>
                    <Controller name={`noSeries.${idx}.startNumber`} control={control} render={({ field: subField }) => (
                        <FormInput
                            ...
                            error={itemErrors?.startNumber?.message}
                        />
                    )} />
                    ...
                    <div>
                        ...
                        <p className="text-xs text-neutral-400 mt-1 dark:text-neutral-500">
                            Live preview of the first generated number
                        </p>
                    </div>
                </TwoCol>
