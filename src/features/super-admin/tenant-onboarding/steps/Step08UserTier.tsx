        ...
        <div className="space-y-0 animate-in fade-in slide-in-from-right-3 duration-300">
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="mb-3">
                                    ...
                                    <p className="text-lg font-bold text-primary-950 mt-2">{tier.range}</p>
                                ...
                                {tier.key !== 'custom' ? (
                                    <div className="mt-auto">
                                        ...
                                        <p className="text-xs text-neutral-500">/month base + ₹{tier.perUserPrice}/user</p>
                                    </div>
                                ) : (
                                    <div className="mt-auto">
                                        <p className="text-lg font-bold text-accent-700">Negotiated Pricing</p>
                                        <p className="text-xs text-neutral-500">Contact Avyren for custom quote</p>
                                    </div>
                                )}
                            </button>
                        );
            ...
            <SectionCard title="Billing Cycle" subtitle="Annual billing includes a 2-month discount (pay for 10, get 12)">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                        ...
                        <div>
                            <p className="text-sm font-bold text-primary-950">Monthly Billing</p>
                            <p className="text-xs text-neutral-500 mt-0.5">Billed every month. Cancel anytime.</p>
                        </div>
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            step8.billingCycle === 'annual'
                                ...
                                : 'border-neutral-200 bg-white hover:border-success-200'
                        )}
                    >
                        ...
                        <div className="w-10 h-10 rounded-xl bg-success-100 flex items-center justify-center text-lg">📆</div>
                        <div>
                            <p className="text-sm font-bold text-primary-950">Annual Billing</p>
                            <p className="text-xs text-neutral-500 mt-0.5">Billed once per year. 2 months free.</p>
                        </div>
            ...
            <SectionCard title="Trial Period" subtitle="Free trial days before billing begins (PRD: 14 days standard)">
                <div className="flex items-center gap-4">
                    ...
                    />
                    <div className="bg-primary-50 border border-primary-200 rounded-xl px-4 py-3 mt-5">
                        <p className="text-xs font-bold text-primary-800">14 days = Standard trial</p>
                ...
                <div className="space-y-3">
                    {resolved.length > 0 && (
                        <div className="space-y-2">
                            <p className="text-xs font-bold text-neutral-500 uppercase tracking-wider">Modules</p>
                            {resolved.map((id) => {
                                const mod = MODULE_CATALOGUE.find((m) => m.id === id);
                                if (!mod) return null;
                                ...
                                return (
                                    <div key={id} className="flex items-center justify-between text-sm">
                                        <span className="text-neutral-700">{mod.icon} {mod.name}</span>
                                        <span className="font-semibold text-primary-800">₹{price.toLocaleString('en-IN')}</span>
                                    </div>
                                );
                            })}
                            <div className="border-t border-neutral-100 pt-2 flex items-center justify-between text-sm font-bold">
                                <span className="text-neutral-700">Modules Subtotal</span>
                                <span className="text-primary-800">₹{moduleCost.toLocaleString('en-IN')}</span>
                            </div>
                        </div>
                    )}
                    ...
                    {currentTier && (
                        <div className="space-y-2">
                            <p className="text-xs font-bold text-neutral-500 uppercase tracking-wider">User Tier</p>
                            <div className="flex items-center justify-between text-sm">
                                <span className="text-neutral-700">{currentTier.label} Tier ({currentTier.range})</span>
