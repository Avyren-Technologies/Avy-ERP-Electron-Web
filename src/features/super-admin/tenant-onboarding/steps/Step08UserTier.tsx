...
export function Step08UserTier() {
    const { step7, step8, setStep8, goNext } = useTenantOnboardingStore();

    const { control, handleSubmit, watch } = useForm<FormData>({
        resolver: zodResolver(schema),
        defaultValues: {
            userTier: step8.userTier,
            customUserLimit: step8.customUserLimit,
            customTierPrice: step8.customTierPrice,
            billingCycle: step8.billingCycle,
            trialDays: step8.trialDays,
        }
    });
                ...
                {userTier === 'custom' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 animate-in fade-in duration-200">
                        <Controller name="customUserLimit" control={control} render={({ field, fieldState }) => (
                            <FormInput label="Custom User Limit" placeholder="e.g. 2500" {...field} value={field.value || ''} type="number" required hint="Maximum concurrent users for this tenant" error={fieldState.error?.message} />
                        )} />
                        <Controller name="customTierPrice" control={control} render={({ field, fieldState }) => (
                            <FormInput label="Custom Monthly Tier Price (₹)" placeholder="e.g. 60000" {...field} value={field.value || ''} type="number" required hint="Negotiated monthly tier base price; excludes module costs" error={fieldState.error?.message} />
                        )} />
                    </div>
                )}
            </SectionCard>
            ...
            <SectionCard title="Trial Period" subtitle="Free trial days before billing begins (PRD: 14 days standard)">
                <Controller name="trialDays" control={control} render={({ field, fieldState }) => (
                    <div className="flex items-center gap-4">
                        <FormInput label="Trial Days" placeholder="14" type="number" {...field} value={field.value || ''} hint="After trial, billing activates automatically unless cancelled" className="max-w-[200px]" error={fieldState.error?.message} />
                    ...
                    <div className="border-t-2 border-primary-200 pt-3 flex items-center justify-between dark:border-primary-800/50">
                        ...
                        <div className="text-right">
                            <p className="text-3xl font-bold text-primary-600">
                                ₹{(billingCycle === 'annual' ? totalAnnual : totalMonthly).toLocaleString('en-IN')}
                            </p>
                            <p className="text-xs text-neutral-500 dark:text-neutral-400">
