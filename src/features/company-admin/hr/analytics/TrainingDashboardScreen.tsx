...
export function TrainingDashboardScreen() {
  ...
  const { data: response, isLoading, error } = useAnalyticsDashboard('training', filters);
  ...
  const handleDrilldown = (type: string) => {
    navigate(`/app/company/hr/analytics/training?drilldown=${type}`);
  };
  ...
  if (!isLoading && !dashboardData?.kpis?.length && !dashboardData?.meta?.lastComputedAt) {
    return (
      <ZeroDataState
        ...
        description="Training analytics will appear once programmes, sessions, and nominations are tracked."
        ctaLabel="Go to Training"
        ctaPath="/app/company/hr/training"
      />
    );
  }
  ...
  return (
    <DashboardShell
      ...
      headerSubtitle="Monitor programme effectiveness, completion rates, and trainer performance."
      headerAside={(
      ...
      loading={isLoading}
      error={error ? 'Failed to load training dashboard' : null}
    >
      ...
      <div>
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <h2 className="text-sm font-bold text-neutral-800 dark:text-neutral-100">Training Trends</h2>
        </div>
      ...
      <div>
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <h2 className="text-sm font-bold text-neutral-800 dark:text-neutral-100">Training Distribution</h2>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
