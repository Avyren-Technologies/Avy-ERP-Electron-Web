...
function RateLimitBadge() {
  ...
  return (
    <div className="flex items-center gap-3 relative overflow-hidden bg-gradient-to-br from-[#F5F8FF] via-white to-[#F0EDFF] dark:from-indigo-950/20 dark:via-neutral-900 dark:to-violet-950/20 rounded-xl border border-neutral-200/60 dark:border-neutral-800 shadow-[0_4px_12px_rgba(0,0,0,0.03)] px-4 py-2.5">
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <>
          <div className="overflow-x-auto rounded-2xl bg-gradient-to-br from-[#F5F8FF] via-white to-[#F0EDFF] dark:from-indigo-950/20 dark:via-neutral-900 dark:to-violet-950/20 border border-neutral-200/60 dark:border-neutral-800 shadow-[0_4px_12px_rgba(0,0,0,0.03)]">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-200/60 dark:border-neutral-700/60">
...
export function ReportsHubScreen() {
  ...
  const catalogCategories = useMemo(() => {
    ...
    return Object.entries(catalogData as Record<string, unknown>).map(([categoryKey, categoryValue]) => {
      // Backend currently returns { [category]: { meta, reports } }, but we also support
      // the older shape { [category]: reports[] } to avoid runtime crashes during transitions.
      const reportList = Array.isArray(categoryValue)
        ? categoryValue
        : (categoryValue as { reports?: unknown })?.reports;

      const safeReports = Array.isArray(reportList)
        ? reportList
        : [];
      ...
      return {
        ...
        reports: safeReports.map((r) => {
          ...
          return {
            ...
            sheets: Array.isArray(report.sheetNames) ? report.sheetNames : [],
          };
        }).filter(r => r.key),
      };
    });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-neutral-50 to-neutral-100/50 dark:from-neutral-950 dark:to-neutral-900/50">
      {/* Header */}
      <div className="relative overflow-hidden bg-gradient-to-br from-[#4A3AFF] via-indigo-500 to-violet-500 dark:from-indigo-800 dark:via-indigo-700 dark:to-violet-700 rounded-2xl">
        {/* Decorative elements */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-4 relative z-20">
        {/* Tabs + Rate Limit */}
        <div className="relative overflow-hidden bg-gradient-to-br from-[#F5F8FF] via-white to-[#F0EDFF] dark:from-indigo-950/20 dark:via-neutral-900 dark:to-violet-950/20 rounded-2xl shadow-[0_4px_12px_rgba(0,0,0,0.03)] border border-neutral-200/60 dark:border-neutral-800 p-1.5 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
