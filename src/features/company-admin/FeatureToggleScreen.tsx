...
import { useFeatureToggleCatalogue } from "@/features/company-admin/api/use-company-admin-queries";
...
export function FeatureToggleScreen() {
    ...
    const { data: catalogueData } = useFeatureToggleCatalogue();
    ...
    // Merge catalogue with user toggles — show all catalogue items with enabled state
    const mergedToggles = useMemo(() => {
        if (catalogue.length === 0) return toggles;

        const toggleMap = new Map<string, FeatureToggle>();
        toggles.forEach((t) => toggleMap.set(t.featureKey, t));
        ...
        return catalogue.map((cat) => {
            ...
            if (existing) {
                return {
                    ...
                    module: existing.module || cat.module,
                    label: existing.label || cat.label,
                };
            }
            // Feature from catalogue but not in user's current toggles
            return {
                id: cat.key,
                featureKey: cat.key,
                label: cat.label,
    ...
    const handleToggle = async (toggle: FeatureToggle) => {
        ...
        setSaving(toggle.id);
        try {
            const updatedToggles = mergedToggles.map((t) =>
            ...
            // Re-fetch toggles to sync state
    ...
    const filtered = mergedToggles.filter((t) => {
        if (!search) return true;
        const s = search.toLowerCase();
        return t.label?.toLowerCase().includes(s) || t.featureKey?.toLowerCase().includes(s) || t.module?.toLowerCase().includes(s);
    });
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {loadingToggles ? (
                ...
                <div className="space-y-6">
                    {Array.from(groupedByModule.entries()).map(([moduleName, items]) => (
                        <div key={moduleName}>
                            <h3 className="text-xs font-bold text-neutral-400 dark:text-neutral-500 uppercase tracking-widest mb-3">{moduleName}</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {items.map((toggle) => {
