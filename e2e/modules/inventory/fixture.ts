...
/**
 * Inventory module test fixture.
 * Provides an InventoryApiClient (worker-scoped, logs in once).
 *
 * Usage in tests:
 *   import { test, expect } from '../../modules/inventory/fixture';
 *   test('my test', async ({ api }) => { ... });
 */
export const test = base.extend<object, { api: InventoryApiClient }>({
  api: [async ({ playwright }, use) => {
    const ctx = await playwright.request.newContext({
      baseURL: process.env.E2E_API_URL || 'http://localhost:3030/api/v1',
    });
    const client = new InventoryApiClient(ctx);
